...
type MockQueries struct {
	createdTarget db.SpiderTarget
	createError   error
}

func (m *MockQueries) CreateTarget(ctx context.Context, params db.CreateTargetParams) (db.SpiderTarget, error) {
	if m.createError != nil {
		return db.SpiderTarget{}, m.createError
	}

	// Simulate successful creation
	target := db.SpiderTarget{
		ID:         1,
